...
void BlinkyPicoWCube::init(int commLEDPin, int commLEDBright, int resetButtonPin)
{
  ...
  if (file) 
  {
      ...
      g_trayType           = lines[6];
      g_trayName           = lines[7];
      g_mqttSubscribeTopic = g_box + "/cube/" + g_trayType + "/" + g_trayName + "/setting";
      g_mqttPublishTopic   = g_box + "/cube/" + g_trayType + "/" + g_trayName + "/reading";
      file.close();
...
void BlinkyPicoWCube::readWebPage()
{
  ...
  if (client) 
  {
    ...
    if (header.indexOf("POST") >= 0)
    {
      ...
      g_trayName           = replaceHtmlEscapeChar(data.substring(data.indexOf("&tryn=")  + 6,data.length()));
      g_mqttSubscribeTopic = g_box + "/cube/" + g_trayType + "/" + g_trayName + "/setting";
      g_mqttPublishTopic   = g_box + "/cube/" + g_trayType + "/" + g_trayName + "/reading";
      if (g_chattyCathy) Serial.println(header);
